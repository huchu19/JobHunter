  ...
  return (
    <button
      ...
      className={`flex h-full w-12 shrink-0 flex-col items-center gap-3 rounded-xl border py-3 transition-colors ${
        isOver
          ? "border-column-border bg-column-bg-hover"
          : "border-column-border bg-column-bg hover:bg-column-bg-hover"
      }`}
    >
      <span
      ...
      />
      <span className="figure rounded-full bg-column-bg-hover px-1.5 py-0.5 text-[11px] font-semibold tabular-nums text-column-text">
        {cards.length}
      </span>
      <span
        className="text-sm font-semibold tracking-wide text-column-text [writing-mode:vertical-rl]"
      >
        {meta.label}
      </span>
    </button>
  ...
  return (
    <div
      ...
      className={`flex h-full w-[300px] shrink-0 flex-col rounded-xl border transition-colors ${
        isOver
          ? "border-column-border bg-column-bg-hover"
          : "border-column-border bg-column-bg"
      }`}
    >
      {/* Sticky column header — count stays visible no matter how far you scroll. */}
      <div className="flex shrink-0 items-center justify-between gap-2 rounded-t-xl border-b border-column-border bg-column-bg-hover px-3 py-2.5">
        <div className="flex items-center gap-2">
          <span
            className="h-2 w-2 rounded-full"
            style={{ backgroundColor: meta.accent }}
          />
          <h3 className="text-sm font-semibold text-column-text">{meta.label}</h3>
          <span className="figure rounded-full bg-column-border px-2 py-0.5 text-xs font-semibold tabular-nums text-column-text">
            {cards.length}
          </span>
        </div>
        <button
          ...
          className="grid h-6 w-6 place-items-center rounded-md text-column-text/40 transition-colors hover:bg-column-border hover:text-column-text"
        >
      ...
      {overflowing && (
        <button
          onClick={jump}
          className="flex shrink-0 items-center justify-center gap-1.5 rounded-b-xl border-t border-column-border bg-column-bg py-1.5 text-[11px] font-semibold text-column-text/55 transition-colors hover:bg-column-bg-hover hover:text-column-text"
        >
          {atBottom ? (
            <>
